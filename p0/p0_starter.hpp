...
public:
  ...
  [[nodiscard]] virtual int GetColumnCount() const = 0;
...
public:
   ...
   * @return The number of columns in the matrix
   */
  [[nodiscard]] int GetColumnCount() const override {
    return this->cols_;
  }
  ...
  void FillFrom(const std::vector<T>& source) override {
    if (source.size() != this->rows_ * this->cols_) {
      throw std::out_of_range("wrong size of `source`");
    }
    auto p = source.begin();
    for (int i = 0; i < this->rows_; i++) {
...
public:
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T>* matrixA, const RowMatrix<T>* matrixB,
    ...
    auto rb = matrixB->GetRowCount();
    auto cb = matrixB->GetColumnCount();
    auto rc = matrixC->GetRowCount();
    auto cc = matrixC->GetColumnCount();
    if (ca != rb) {
      return nullptr;
    }
    if (ra != rc || cb != cc) {
    ...
    auto res = std::make_unique<RowMatrix<T>>(rc, cc);
    for (int i = 0; i < ra; i++) {
      for (int j = 0; j < cb; j++) {
        T val = 0;
        for (int k = 0; k < ca; k++) {
    ...
    for (int i = 0; i < rc; i++) {
      for (int j = 0; j < cc; j++) {
        res->SetElement(i, j, res->GetElement(i, j) + matrixC->GetElement(i, j));
      }
    }
    return std::move(res);
  }
